...
namespace BidCat.API
{
	public class BiddingController : ApiProvider
	{
		...
		public override IEnumerable<ApiCommand> Commands => new List<ApiCommand>
		{
			...
			new ApiCommand("GetReservedMoneyAuction", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetReservedMoney(args.Skip(1).Count() == 1 ? JsonConvert.DeserializeObject<IEnumerable<int>>(args.Skip(1).First()) : args.Skip(1).Any() ? args.Skip(1).Select(int.Parse) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter>{ new ApiParameter("Auction ID"), new ApiParameter("User Ids", "IEnumerable<int>") }, "Finds the total reserved money for a given user."),
			...
			new ApiCommand("GetAllBidsOrdered", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetAllBidsOrdered().Result, new List<ApiParameter>{ new ApiParameter("Auction ID") }, "Returns all bids as List<Tuple<string, List<Tuple<int, int>>>>, ordered by ranking (first=winner)"),
			new ApiCommand("GetBidsForUser", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetBidsForUser(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter> { new ApiParameter("Auction ID"), new ApiParameter("User ID") }, "Gets all bids on every lot for the given user"),
			new ApiCommand("GetBidsForUsers", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetBidsForUsers(args.Skip(1).Count() == 1 ? JsonConvert.DeserializeObject<IEnumerable<int>>(args.Skip(1).First()) : args.Skip(1).Any() ? args.Skip(1).Select(int.Parse) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter> { new ApiParameter("Auction ID"), new ApiParameter("User IDs", "IEnumerable<int>") }, "Gets all bids on every lot for the given users"),
			...
			new ApiCommand("MakeTransaction", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).MakeTransaction(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("User ID was not specified"), args.Skip(2).Any() ? int.Parse(args.Skip(2).First()) : throw new ApiError("Amount was not specified"), args.Skip(3).Any() ? JsonConvert.DeserializeObject<Dictionary<string, object>>(args.Skip(3).First()) : new Dictionary<string, object>()).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("User ID"), new ApiParameter("Amount"), new ApiParameter("Extra", "Dictionary<string, object>", true) }, "Makes a transaction for the specified user"),
			new ApiCommand("MakeTransactions", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).MakeTransactions(args.Skip(1).Any() ? JsonConvert.DeserializeObject<List<Tuple<int, int, Dictionary<string, object>>>>(args.Skip(1).First()) : throw new ApiError("Transactions were not specified")).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("Transactions", "List<Tuple<int, int, Dictionary<string, object>>>") }, "Makes many transactions for the specified users, the first item of the tuple is the User ID, the second is the amount to change by, and the third is any additional records to include"),
			new ApiCommand("GetReservedMoneyBank", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).GetReservedMoney(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("User ID") }, "Gets the total reserved money for the specified user"),
			new ApiCommand("GetReservedMoneyBankMultiple", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).GetReservedMoneyMultiple(args.Skip(1).Any() ? JsonConvert.DeserializeObject<IEnumerable<int>>(args.Skip(1).First()) : throw new ApiError("User IDs were not specified")).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("User IDs", "IEnumerable<int>") }, "Gets the total reserved money for multiple users"),
			new ApiCommand("GetAvailableMoneyBank", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).GetAvailableMoney(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("User ID") }, "Gets the total available money for the specified user"),
			new ApiCommand("GetAvailableMoneyBankMultiple", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).GetAvailableMoneyMultiple(args.Skip(1).Any() ? JsonConvert.DeserializeObject<IEnumerable<int>>(args.Skip(1).First()) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter>{ new ApiParameter("Bank ID"), new ApiParameter("User IDs", "IEnumerable<int>") }, "Gets the total available money for multiple users"),
			...
			new ApiCommand("RegisterTimeSoftCooldown", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).RegisterSoftCooldown(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("Cooldown length was not specified"), args.Skip(2).Any() ? int.Parse(args.Skip(2).First()) : throw new ApiError("Minimum bid was not specified"), false).Wait(), new List<ApiParameter>{ new ApiParameter("Auction ID"), new ApiParameter("Cooldown Length"), new ApiParameter("Minimum Bid") }, "Registers a soft cooldown for the given auction, lengths should be given in milliseconds."),
			new ApiCommand("RegisterLotSoftCooldown", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).RegisterSoftCooldown(args.Skip(1).Any() ? int.Parse(args.Skip(1).First()) : throw new ApiError("Cooldown length was not specified"), args.Skip(2).Any() ? int.Parse(args.Skip(2).First()) : throw new ApiError("Minimum bid was not specified"), true).Wait(), new List<ApiParameter>{ new ApiParameter("Auction ID"), new ApiParameter("Cooldown Length"), new ApiParameter("Minimum Bid") }, "Registers a soft cooldown for the given auction, lengths should be given in amounts of lots."),
			...
			new ApiCommand("GetAuctionCooldown", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetCooldown(args.Skip(1).Any() ? args.Skip(1).First() : throw new ApiError("Lot ID was not specified")).Result, new List<ApiParameter> { new ApiParameter("Auction ID"), new ApiParameter("Lot ID", "string") }, "Gets the cooldown for a specified lot"),
			new ApiCommand("GetAuctionCooldowns", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetCooldowns(args.Skip(1).Any() ? JsonConvert.DeserializeObject<List<string>>(args.Skip(1).First()) : throw new ApiError("Lot IDs were not specified")).Result, new List<ApiParameter> { new ApiParameter("Auction ID"), new ApiParameter("Lot IDs", "IEnumerable<string>") }, "Gets the cooldowns for multiple lots"),
			new ApiCommand("GetBankCooldowns", args => GetBank(args.Any() ? int.Parse(args.First()) : throw new ApiError("Bank ID was not specified")).GetCooldowns(), new List<ApiParameter> { new ApiParameter("Bank ID") }, "Gets all cooldowns for the specified bank")
		};
