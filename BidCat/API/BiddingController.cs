...
namespace BidCat.API
{
	public class BiddingController : ApiProvider
	{
		...
		public override IEnumerable<ApiCommand> Commands => new List<ApiCommand>
		{
			...
			new ApiCommand("GetReservedMoneyAuction", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetReservedMoney(args.Skip(1).Count() == 1 ? JsonConvert.DeserializeObject<IEnumerable<int>>(args.Skip(1).First()) : args.Skip(1).Any() ? args.Skip(1).Select(int.Parse) : throw new ApiError("User ID was not specified")).Result, new List<ApiParameter>{ new ApiParameter("Auction ID"), new ApiParameter("User Id") }, "Finds the total reserved money for a given user."),
