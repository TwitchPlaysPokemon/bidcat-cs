...
namespace BidCat.API
{
	public class BiddingController : ApiProvider
	{
		...
		public override IEnumerable<ApiCommand> Commands => new List<ApiCommand>
		{
			...
			new ApiCommand("GetAuctionCooldown", args => GetAuction(args.Any() ? int.Parse(args.First()) : throw new ApiError("Auction ID was not specified")).GetCooldown(args.Skip(1).Any() ? args.Skip(1).First() : throw new ApiError("Lot ID was not specified")).Wait(), new List<ApiParameter> { new ApiParameter("Auction ID"), new ApiParameter("Lot ID", "string") }, "Gets the cooldown for a specified lot"),
